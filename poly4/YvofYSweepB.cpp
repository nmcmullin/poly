...
#include "flag.h"
#define PI 3.14159
void FreeSurfaceCalc::FluxCalcYsweepBIIrevIII(int &I, int &J,float &ALPHA, float &BETA, float &ST, float &SR, float &SB, float &SL)
{
if(ALPHA<(PI/4))
{
	...
	if(v[J*ncols+I]>0.0)
	{
		...
		{
		Ft[J*ncols+I]=0.5*(v[J*ncols+I]*dt-(1.0-SL)*dy)*(v[J*ncols+I]*dt-(1.0-SL)*dy)*(1.0/tan(BETA));
		}
	}
	if(v[(J-1)*ncols+I]<0.0)
	{
		...
		{
		Fb[J*ncols+I]=0.5*(abs(v[(J-1)*ncols+I])*dt*(2.0-abs(v[(J-1)*ncols+I])*dt/(SL*dy)))*SB*dx;
		}	
	}
	}
	...
	if(v[J*ncols+I]>0.0)
	{
		...
		{
		Ft[J*ncols+I]=v[J*ncols+I]*dt*dx-(1.0-C[J*ncols+I])*dx*dy;
		}
		else
		{
		Ft[J*ncols+I]=v[J*ncols+I]*dt*(ST*dx+0.5*v[J*ncols+I]*dt*(1.0/tan(BETA)));
	...
	if(v[(J-1)*ncols+I]<0.0)
	{
		...
		{
		Fb[J*ncols+I]=abs(v[(J-1)*ncols+I])*dt*dx-(0.5*(abs(v[(J-1)*ncols+I])*dt-SR*dy)*(abs(v[(J-1)*ncols+I])*dt-SR*dy)*(1.0/tan(BETA)));
		}
	}
	}
}
else
{
	...
	if(v[J*ncols+I]>0.0)
	{
		...
		else if(v[J*ncols+I]*dt<=(1.0-SR)*dy)
		{
		Ft[J*ncols+I]=0.5*(v[J*ncols+I]*dt-(1.0-SL)*dy)*(v[J*ncols+I]*dt-(1.0-SL)*dy)*(1.0/tan(BETA));
		}
		else
	...
	if(v[(J-1)*ncols+I]<0.0)
	{
		...
		else if(abs(v[(J-1)*ncols+I])*dt<=SL*dy)
		{
		Fb[J*ncols+I]=abs(v[(J-1)*ncols+I])*dt*dx-0.5*(abs(v[(J-1)*ncols+I])*dt-SR*dy)*(abs(v[(J-1)*ncols+I])*dt-SR*dy)*(1.0/tan(BETA));
		}
		else
