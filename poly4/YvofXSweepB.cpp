...
void FreeSurfaceCalc::FluxCalcXsweepBIIrevIII(int &I, int &J,float &ALPHA, float &BETA, float &ST, float &SR, float &SB, float &SL)
{
if(ALPHA<(PI/4))
{
	...
	if(u[J*ncols+I]>0.0)
	{
		...
		{
		Fr[J*ncols+I]=0.5*(u[J*ncols+I]*dt-(1.0-SB)*dx)*(u[J*ncols+I]*dt-(1.0-SB)*dx)*tan(BETA);
		}
	}
	if(u[J*ncols+I-1]<0.0)
	{
		...
		{
		Fl[J*ncols+I]=0.5*abs(u[J*ncols+I-1])*dt*(2.0-abs(u[J*ncols+I-1])*dt/(SB*dx))*SL*dy;
		}
	}
	}
	...
	if(u[J*ncols+I-1]<0.0)
	{
		...
		{
		Fl[J*ncols+I]=abs(u[J*ncols+I-1])*dt*dy-0.5*(abs(u[J*ncols+I-1])*dt-ST*dx)*(abs(u[J*ncols+I-1])*dt-ST*dx)*tan(BETA);
		}
		else
		{
		Fl[J*ncols+I]=C[J*ncols+I]*dx*dy;
	...
	if(u[J*ncols+I-1]<0.0)
	{
		...
		{
		Fl[J*ncols+I]=abs(u[J*ncols+I-1])*dt*dy-0.5*tan(BETA)*(abs(u[J*ncols+I-1])*dt-ST*dx)*(abs(u[J*ncols+I-1])*dt-ST*dx);
		}
	}
	}
}
