...
void FreeSurfaceCalc::FluxCalcXsweepA(int &I, int &J,float &ALPHA, float &BETA, float &ST, float &SR, float &SB, float &SL)
{
if(ALPHA<(PI/4))
{
	...
	if(u[J*ncols+I]>0.0)
	{
		...
		{
		Fr[J*ncols+I]=0.5*u[J*ncols+I]*dt*(2.0-u[J*ncols+I]*dt/(SB*dx))*SR*dy;
		}
	}
	if(u[J*ncols+I-1]<0.0)
	{
		...
		{
		Fl[J*ncols+I]=0.5*(abs(u[J*ncols+I-1])*dt-(1.0-SB)*dx)*(abs(u[J*ncols+I-1])*dt-(1.0-SB)*dx)*tan(BETA);
		}
	}
	}
	...
	if(u[J*ncols+I]>0.0)
	{
		...
		{
		Fr[J*ncols+I]=u[J*ncols+I]*dt*dy-0.5*tan(BETA)*(u[J*ncols+I]*dt-ST*dx)*(u[J*ncols+I]*dt-ST*dx);
		}
	}
	if(u[J*ncols+I-1]<0.0)
	{
...
else
{
	...
	if(u[J*ncols+I]>0.0)
	{
		...
		{
		Fr[J*ncols+I]=u[J*ncols+I]*dt*dy-0.5*(u[J*ncols+I]*dt-ST*dx)*(u[J*ncols+I]*dt-ST*dx)*tan(BETA);
		}
		else
		{
		Fr[J*ncols+I]=C[J*ncols+I]*dx*dy;
	...
	if(u[J*ncols+I-1]<0.0)
	{
		...
		else if(abs(u[J*ncols+I-1])*dt<=ST*dx)
		{
		Fl[J*ncols+I]=0.5*(abs(u[J*ncols+I-1])*dt-(1.0-SB)*dx)*(abs(u[J*ncols+I-1])*dt-(1.0-SB)*dx)*tan(BETA);
		}
		else
